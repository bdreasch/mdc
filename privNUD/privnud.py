import numpy as np
import math
import privNUD.grid_generate as GridGen
import privNUD.estimate_method as EstMeth
import privNUD.frequency_oracle as FreOra
import itertools
import privNUD.choose_granularity
import privNUD.dynamic_tree_def as Dyna
...
#
#     def generate_attribute_group_2(self):
#         attribute_group_list = []
#         attribute_list = [i for i in range(self.args.attribute_num)]
#         for tmp_attribute in attribute_list:
#             attribute_group_list.append((tmp_attribute,))
...
#         return
#
#     def group_attribute_2(self):
#         self.generate_attribute_group_2()  # 保留二维网格，计算响应矩阵用
#         return
#
#
...
#
#
#     def get_LDP_Grid_set_divide_user_2(self, user_record):
#         #print("动态树——HDG is working...")
#         # 所有维度上的 1维动态树 叶节点
#
#         # 修改用户分配!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#         point_d_Cd2 = self.args.user_num // self.group_num * self.args.attribute_num
#         user_record_1_way = user_record[0:point_d_Cd2]
#         user_record_2_way = user_record[point_d_Cd2:]
#         # user_record_1_way = user_record[0:self.args.user_num // 5 * 4]
#         # user_record_2_way = user_record[self.args.user_num // 5 * 4:]
#
#         dyna_noise_data = Dyna.main(user_record=user_record_1_way, group_num=self.args.attribute_num, epsilon=self.args.epsilon, domain=self.args.domain_size)
#
#         # update the perturbed_count of each cell
#
#         self.LDP_mechanism_list_divide_user = []  # intialize for each time to randomize user data
#         for j in range(self.args.attribute_num, self.group_num):  # initialize LDP mechanism for each attribute group
#             tmp_Grid = self.Grid_set[j]   # the i-th Grid
#             tmp_domain_size = len(tmp_Grid.cell_list)
#
#             tmp_LDR = FreOra.OUE(domain_size=tmp_domain_size, epsilon= self.args.epsilon, sampling_factor=self.group_num, args=self.args)
#             # tmp_LDR = FreOra.OLH(domain_size=tmp_domain_size, epsilon= self.args.epsilon, sampling_factor=self.group_num, args=self.args)
#
#             self.LDP_mechanism_list_divide_user.append(tmp_LDR)   # 只有Cd2个
#
#         for i in range(len(user_record_2_way)):  # 分配给二维网格的用户，分成Cd2组
#             tmp_user_granularity = math.ceil(len(user_record_2_way) / (self.group_num - self.args.attribute_num))
#             group_index_of_user = i // tmp_user_granularity     # 分配：每个用户选一个组号
#             j = group_index_of_user + self.args.attribute_num   # 每个用户对应的网格j
#
#             # to count the user num of each group
...
#             tmp_Grid = self.Grid_set[j]  # the j-th Grid
#
#             if len(tmp_Grid.attribute_set) == 1:  # 一维：用动态树的叶节点
#                 dim_index = tmp_Grid.attribute_set[0]
#                 for k in range(len(tmp_Grid.cell_list)):
#                     tmp_Grid.cell_list[k].perturbed_count = dyna_noise_data[dim_index][k] * self.args.user_num  # 频数：投射到n
#             else:
#                 tmp_LDP_mechanism = self.LDP_mechanism_list_divide_user[j - self.args.attribute_num]
#                 tmp_LDP_mechanism.operation_aggregate()        # oue加噪
#                 for k in range(len(tmp_Grid.cell_list)):
#                     tmp_Grid.cell_list[k].perturbed_count = tmp_LDP_mechanism.aggregated_count[k]
#
#         return
...
#         return flag
#
#     def get_answer_range_query_attribute_group_list(self, selected_attr_index_list):
#         answer_range_query_attribute_group_index_list = []
#         answer_range_query_attribute_group_list = []
#         for tmp_Grid in self.Grid_set:
#             #note that here we judge if tmp_Grid.attribute_set belongs to selected_attr_index_list
#             if self.judge_sub_attribute_list_in_attribute_group(tmp_Grid.attribute_set, selected_attr_index_list):
...
#         self.get_answer_range_query_attribute_group_list(range_query.selected_attr_index_list)
#
#         for k in answer_range_query_attribute_group_index_list:
#             tmp_Grid = self.Grid_set[k]
#             Grid_range_attr_node_list = []
#             for tmp_attribute in tmp_Grid.attribute_set:
#                 Grid_range_attr_node_list.append(range_query.attr_node_list[tmp_attribute])
#             t_Grid_ans.append(tmp_Grid.answer_range_query_with_weight_update_matrix(Grid_range_attr_node_list))  # 输入相关网格G的相关点
#
...
#             tans_weighted_update = self.answer_range_query(tmp_range_query)  # 输入一个λ查询
#             self.weighted_update_answer_list.append(tans_weighted_update)
#         return
#
#     def answer_range_query_list_2(self, range_query_list):
#         self.weighted_update_answer_list = []
#         for tmp_range_query in range_query_list:
#             tans_weighted_update = self.answer_range_query(tmp_range_query)  # 输入一个λ查询
#             self.weighted_update_answer_list.append(tans_weighted_update)
...
class PrivNud:       # 我
    ...
    def set_granularity_1_2_way(self):
        chooseGran = privNUD.choose_granularity.choose_granularity_beta(args= self.args)
        # tmp_g1 = chooseGran.get_1_way_granularity_for_HDG(ep= self.args.epsilon)
        tmp_g1 = self.args.domain_size             # 我
        tmp_g2 = chooseGran.get_2_way_granularity_for_HDG_2(ep= self.args.epsilon)   # 用户分配，改变g2
        self.granularity_1_way = chooseGran.get_rounding_to_pow_2(gran= tmp_g1)
        self.granularity = chooseGran.get_rounding_to_pow_2(gran= tmp_g2)
    ...
    def get_consistent_grid_set(self):
        ...
        for i in range(self.args.consistency_iteration_num_max):
            for tmp_grid in self.Grid_set:
                tmp_grid.get_consistent_grid_iteration()  # 多轮非负处理
    ...
    def weighted_update_iteration(self, grid_1_way_list = None, grid_2_way = None):
        ...
        for tmp_cell in grid_2_way.cell_list:
            ...
            grid_2_way.weighted_update_matrix[x_lower_bound:x_upper_bound, y_lower_bound:y_upper_bound] = grid_2_way.weighted_update_matrix[x_lower_bound:x_upper_bound, \
                                                                                                          y_lower_bound:y_upper_bound] / tmp_sum * tmp_cell.consistent_count
            # normalization
            if np.sum(grid_2_way.weighted_update_matrix) != 0:  # 我
                grid_2_way.weighted_update_matrix = grid_2_way.weighted_update_matrix / np.sum(grid_2_way.weighted_update_matrix) * self.args.user_num
    ...
    def get_wu_for_2_way_group(self):
        for tmp_grid in self.Grid_set:
            if len(tmp_grid.attribute_set) == 2:
                ...
                for i in range(self.args.wu_iteration_num_max):  # 最大循环次数
                    weighted_update_matrix_before = np.copy(tmp_grid.weighted_update_matrix)
                    self.weighted_update_iteration(grid_1_way_list, tmp_grid)  # 输入一维网格、当前二维网格，更新M矩阵
                    weighted_update_matrix_delta = np.sum(np.abs(tmp_grid.weighted_update_matrix - weighted_update_matrix_before))
                    if weighted_update_matrix_delta < 1:
    ...
    def generate_attr_group(self):
        attribute_group_list = []
        attribute_list = [i for i in range(self.args.attr_num)]
        for tmp_attribute in attribute_list:
            attribute_group_list.append((tmp_attribute,))
        attribute_group_2_way_list = list(itertools.combinations(attribute_list, self.group_attribute_num))
        for tmp_attribute_group_2_way in attribute_group_2_way_list:
    ...
    def run_ldp(self, user_record):
        ...
        # user_record_1_way = user_record[0:point_d_Cd2]
        # user_record_2_way = user_record[point_d_Cd2:]
        user_record_1_way = user_record[0:int(self.args.user_num * self.args.user_alpha)]
        user_record_2_way = user_record[int(self.args.user_num * self.args.user_alpha):]

        dyna_noise_data = Dyna.main(user_record=user_record_1_way, group_num=self.args.attr_num, epsilon=self.args.epsilon, domain=self.args.domain_size)

        # update the perturbed_count of each cell

        self.LDP_mechanism_list_divide_user = []  # intialize for each time to randomize user data
        for j in range(self.args.attr_num, self.group_num):  # initialize LDP mechanism for each attribute group
            tmp_Grid = self.Grid_set[j]   # the i-th Grid
            tmp_domain_size = len(tmp_Grid.cell_list)
        ...
        for i in range(len(user_record_2_way)):
            try:
                tmp_user_granularity = math.ceil(len(user_record_2_way) / (self.group_num - self.args.attr_num))
                group_index_of_user = i // tmp_user_granularity
                j = group_index_of_user + self.args.attr_num
                ...
                print("privnud run_ldp 1: {}".format(exc))
        ...
        for j in range(self.group_num):
            ...
            try:
                ...
                if len(tmp_Grid.attribute_set) == 1:
                    dim_index = tmp_Grid.attribute_set[0]
                    for k in range(len(tmp_Grid.cell_list)):
                        tmp_Grid.cell_list[k].perturbed_count = dyna_noise_data[dim_index][k] * self.args.user_num
                else:
                    tmp_LDP_mechanism = self.LDP_mechanism_list_divide_user[j - self.args.attr_num]
                    tmp_LDP_mechanism.operation_aggregate()        # oue加噪
                    for k in range(len(tmp_Grid.cell_list)):
                        tmp_Grid.cell_list[k].perturbed_count = tmp_LDP_mechanism.aggregated_count[k]
            except Exception as exc:
                print("privnud run_ldp 2: {}".format(exc))

        return

    def judge_sub_attribute_list_in_attribute_group(self, sub_attribute_list, attribute_group):
        if len(sub_attribute_list) == 1:
            return sub_attribute_list == attribute_group
        flag = True
        for sub_attribute in sub_attribute_list:
            if sub_attribute not in attribute_group:
                flag = False
    ...
    def answer_range_query(self, range_query):
        ...
        try:
            ...
            print('answer_range_query privnud')
