...
def answer_queries(QUERY_TIME, query_path, space_true_value, user_scale, domain, real_node, consis_MSE_list):
    ...
    for times in range(0, QUERY_TIME):
        query_MIN_dim1 = query_list[times][0]
        query_MAX_dim1 = query_list[times][1]

        num = query(query_list[times], Domain=domain, dim=1, node=real_node)
...
if __name__ == '__main__':
    ...
    for epsilon in eps_list:
        ...
        for loop in range(0, Loop):
            ...
            answer_queries(QUERY_TIME=1000, query_path=query_path,
                           space_true_value=space_true_value,
                           user_scale=users,
                           domain=domain, real_node=real_node,
                           consis_MSE_list=consistency_MSE_list)

        consistency_MSE_list = np.array(consistency_MSE_list)
        print('e: {} mean_MSE: {}'.format(epsilon, np.mean(consistency_MSE_list)))
