        ...
        if not node.data.divide_flag:
            ...
            for j in range(0, len(node.data.interval), 2):
                if node.data.interval[j + 1] - node.data.interval[j] > 1:
                    ...
                    TempItem0[j + 1] = (node.data.interval[j + 1] - node.data.interval[j]) //\
                                       branch[j//2] + node.data.interval[j]
                else:
                    TempItem0[j] = node.data.interval[j]
            ...
            for item1 in translation_vector:
                ...
                ahead_tree.create_node(node_name, node_name, parent=node.identifier,
                                       data=Nodex(0, True, 0, node_interval))
                layer_index += 1

# Step3: Noisy Frequency Construction (NFC)
def ahead_tree_construction(ahead_tree, ahead_tree_height, theta, branch,
...
def node_frequency_aggregation(ahead_tree, user_dataset, epsilon):
    ...
    for i, node in enumerate(ahead_tree.leaves()):
        if node.data.count == 0:
            ...
            node.data.frequency = ((node.data.count * node.data.frequency) +
                                   noisy_frequency[i]) / (node.data.count + 1)
            node.data.count += 1


def main_fun(repeat_time, data_path, domain_size, branch, ahead_tree_height, theta, real_frequency,
             query_interval_table, epsilon, data_name, data_size_name, domain_name):
    repeat = 0
    MAEDict = {'rand': []}
    while repeat < repeat_time:
        ...
        ahead_tree.create_node('Root', 'root',
                               data=Nodex(1, True, 1, np.array([0, domain_size[0], 0, domain_size[1]])))

        # construct sub-domain partition vectors
        translation_vector = construct_translation_vector(domain_size, branch)

        # build a tree structure
        ahead_tree_construction(ahead_tree, ahead_tree_height, theta, branch,
                                translation_vector, user_dataset_partition, epsilon)
...
def run_eps(epsilon):
    ...
    if os.path.exists(real_frequency_path):
        ...
        real_frequency = realfreq.real_frequency_generation(dataset, data_size, domain_size,
    ...
    # running
    # fix the [query_interval_table] if there is more than one query
    mae = main_fun(repeat_time, data_path, domain_size, branch, ahead_tree_height, theta, real_frequency,
             query_interval_table, epsilon, data_name, data_size_name, domain_name)
